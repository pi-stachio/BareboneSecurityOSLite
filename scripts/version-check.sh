#!/bin/bash
# LFS host system requirements check (equivalent to the book's version-check.sh).
export LC_ALL=C
...
ver_check Texinfo makeinfo 5.0
...
echo "Compiler check:"
if printf "int main(){}" | g++ -x c++ - -o /tmp/lfs-cc-test 2>/dev/null; then
    echo "OK:    g++ works";
else
    echo "ERROR: g++ does NOT work";
fi
rm -f /tmp/lfs-cc-test
